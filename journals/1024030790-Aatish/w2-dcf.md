...
**Worked with:** Aatish Kumar Sahu (1024030790)
...
Aatish owns the engine/testing track, so we paired on this week.

## What I did (with Aatish)
...
- **Aatish designed the correctness baseline**: he derived and hand-checked the
  reference case (`baseFreeCashFlow=100, growthRate=wacc=10%, terminalGrowth=2%,
  ...
  `baseFreeCashFlow` — easy to verify by hand and a strong regression guard.
  His hand-worked numbers: `pvOfProjection=300`, `terminalValue=1697.025`,
  `pvOfTerminalValue=1275`, `enterpriseValue=1575`, `equityValue=1525`,
  `intrinsicValuePerShare=152.5`.
- I added `vitest` to the frontend and encoded Aatish's reference case (plus
  both invalid-assumptions cases and a purity check) as the first unit tests in
  `frontend/src/domain/runDCF.test.ts`, then wired `npm run test` into CI
  (`.github/workflows/ci.yml`) alongside lint and build, and into
  `DEVELOPMENT.md`'s checks table.
...
## Problems & fixes
- None — the engine matched Aatish's hand-worked example on the first run once
  the terminal-value discount factor was pinned to the *final* projection year
  (not a separate `projectionYears + 1` factor).

## Verification
- `npm run lint` OK - `npm run test` OK (5/5) - `npm run build` OK.
- Engine output cross-checked against Aatish's independent hand calculation.
